...
{
	while(argv->curr > 0 && argv->argv[0] && argv->argv[0][0] && !is_char_in_set(argv->argv[0][0], OPERATORS))
	{
		if(tree->argv_curr == tree->argv_cap - 1)
		{
			tree->argv_for_func = (char **)realloc_two_d((void **)tree->argv_for_func, tree->argv_cap - 1);
			if(tree->argv_for_func == NULL)
				return (0);
		}
